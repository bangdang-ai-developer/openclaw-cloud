...
- Name: landing-page.spec.ts >> OpenClaw Cloud - Landing Page Tests >> should be mobile responsive
- Location: tests\landing-page.spec.ts:82:7

# Error details

```
Error: locator.scrollIntoViewIfNeeded: Error: strict mode violation: locator('text=Bảng giá') resolved to 3 elements:
    1) <a href="#pricing" class="text-gray-600 hover:text-blue-600">Bảng giá</a> aka getByRole('banner').getByText('Bảng giá')
    ...
    3) <a href="#pricing" class="hover:text-white">Bảng giá</a> aka getByRole('link', { name: 'Bảng giá' })
